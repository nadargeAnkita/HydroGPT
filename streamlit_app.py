...
This version runs standalone without FastAPI (direct function calls).
Set main module to: streamlit_app.py in Streamlit Cloud settings.
"""
...
st.markdown('<div class="sub-header">AI-Driven Hydropower Inflow Forecasting · ML + RAG + ReAct Agent</div>',
            unsafe_allow_html=True)
...
with c1: st.success("✅ App Running") if True else st.error("❌ App Error")
with c2: st.success("✅ ML Models") if models_ok else st.warning("⚠️ Models not trained yet")
                    ...
                    try:
                        ...
                        if forecast:
                            ...
                            st.download_button("⬇️ Download Forecast CSV", csv,
                                               "hydrogpt_forecast.csv", "text/csv")
                    ...
                    messages = [
                        {
                            ...
                            "content": (
                                ...
                                "inflow forecasting. Answer clearly using the provided context."
                            )
                        },
                        {
                            "role": "user",
...
with tab3:
    ...
    try:
        ...
        if metrics:
            ...
            st.info("No trained model metrics found. Run training scripts first.")
            st.markdown("**Phase 1 XGBoost Results (from report):**")
            st.table(pd.DataFrame([{
                "Model": "XGBoost", "RMSE": 15.593, "MAE": 12.461, "R²": 0.862
            }]))
...
# ─────────────────────────────────────────
st.divider()
st.caption("HydroGPT · AI-Driven Hydropower Forecasting · ML + RAG + ReAct Agent | Final Year Project")
